const init = () => {
  ...
  firebase.initializeApp(firebaseConfig);
    ...
    view.setActiveScreen('aboutWindows');
    noname();
...
const noname1 = async () => {
    await firebase.database().ref("messages").on("child_removed", function (snapshot) {
    // remove message node
    document.getElementById("message-" + snapshot.key).innerText = "This comment has been removed";
});
}
const noname = async () => {
    await firebase.database().ref("messages").on("child_added", function (snapshot) {
        var cmtcontent = "";
        ...
        cmtcontent += "</div>"
        cmtcontent += "</div>"
        document.getElementById("messages").innerHTML += cmtcontent;
