const init = () => {
    ...
    view.setActiveScreen('Rainmeter');
    noname();
...
function sendMessage() {
    ...
    // save in database
    firebase.database().ref("messages12").push().set({
...
function deleteMessage(self) {
    ...
    // delete message
    firebase.database().ref("messages12").child(messageId).remove();
    alert("Comment deleted!")
    noname1();
    location.reload(); 
}
const noname1 = async () => {
    await firebase.database().ref("messages12").on("child_removed", function (snapshot) {
    // remove message node
    document.getElementById("message-" + snapshot.key).innerText = "This comment has been removed";
});
}
const noname = async () => {
    await firebase.database().ref("messages12").on("child_added", function (snapshot) {
        var cmtcontent = "";
